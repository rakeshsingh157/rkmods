...
async function getStats() {
  try {
    const appCountRes = await pool.query('SELECT COUNT(*) FROM apps');
    const downloadCountRes = await pool.query('SELECT COUNT(*) FROM downloads');
    const appCount = parseInt(appCountRes.rows[0].count);
    const downloadCount = parseInt(downloadCountRes.rows[0].count);
    return {
